...
| `positions/importFromImage/ImportPositionFromImageScreen.kt` | Import a position from a board screenshot: pick image (`OpenDocument`), draggable crop frame with an 8×8 alignment grid, "white at bottom" toggle, recognition via `BoardImageImportService` + `BoardImageTemplates`, board preview with warnings, then hands the FEN to `PositionSearch`. Entry point: `ImportImageIconButton` in the `PositionSearchScreen` top bar. |
...
| `BoardImageRecognizer.kt` | Screenshot-of-a-2D-board → FEN piece placement, no ML. Pure pixel pipeline over `PixelGrid` (`Bitmap` converted once at the boundary via `Bitmap.toPixelGrid()`); type = nearest template by 1−IoU + glyph-height + horizontal-asymmetry terms over filled, bbox-normalised glyph masks; colour = bright-ink share of the eroded glyph core. A square that clears the emptiness threshold but whose best match scores below `MIN_PIECE_CONFIDENCE` (0.35) is re-classified as empty — this drops coordinate-label debris that leaks into a cell under a slightly misaligned manual crop (real pieces score ≥ 0.69). Calibrated green against all three screenshots in `test positions/` (`BoardImageRecognizerCalibrationTest`, plain JVM + ImageIO). See `PLAN_BOARD_IMAGE_RECOGNITION.md`. |
