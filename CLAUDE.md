...
| `positions/importFromImage/ImportPositionFromImageScreen.kt` | Import a position from a board screenshot: pick image (`OpenDocument`), square framing viewport with an 8×8 alignment grid that the image is pinch-zoomed and panned behind, compact "white at bottom" toggle right under the picker, recognition via `BoardImageImportService` + `BoardImageTemplates` from a bottom-pinned action bar, board preview with warnings, then hands the FEN to `PositionSearch`. Entry point: `ImportImageIconButton` in the `PositionSearchScreen` top bar. |
...
| File | Role |
|---|---|
| `AppIcons.kt` | **All shared styled icon composables live here.** Currently: `SettingsIconButton` (Settings icon, `TrainingAccentTeal` tint), `HintIconButton` (Lightbulb icon, `TrainingAccentTeal` tint, configurable size), `ImportImageIconButton` (AddPhotoAlternate icon, `TrainingTextPrimary` tint). **Always add new icon composables here — never define them inline in a screen or component file.** |
...
| `BoardImageRecognizer.kt` | Screenshot-of-a-2D-board → FEN piece placement, no ML. Pure pixel pipeline over `PixelGrid` (`Bitmap` converted once at the boundary via `Bitmap.toPixelGrid()`); type = nearest template by 1−IoU + glyph-height + horizontal-asymmetry + row-profile + head-profile terms over filled, bbox-normalised glyph masks; colour = bright-ink share of the eroded glyph core. A square that clears the emptiness threshold but whose best match scores below `MIN_PIECE_CONFIDENCE` (0.10) is re-classified as empty — this drops coordinate-label debris that leaks into a cell under a slightly misaligned manual crop (real pieces score ≥ 0.19, debris 0.00). **Confidence is `1 − best distance`, so adding or reweighting any term rescales it — `MIN_PIECE_CONFIDENCE` and `BoardImageImportService.LOW_CONFIDENCE` must be re-measured together with any metric change.** Calibrated against all four screenshots in `test positions/` (`BoardImageRecognizerCalibrationTest`, plain JVM + ImageIO): three exact, the app's own board 62/64. See `PLAN_BOARD_IMAGE_RECOGNITION.md`. |
| `BoardImageTemplates.kt` | Lazy process-lifetime cache of the recognizer's piece templates. `get(context)` decodes `assets/reference_board.png` (clean start position cropped to the board edges) on first call, builds templates via `buildTemplatesFromReferenceBoard`, recycles the bitmap. |
| `BoardImageImportService.kt` | Import-from-screenshot flow logic over `BoardImageRecognizer`: `decodeBoardImage` (content URI → downsampled Bitmap, max side 2048), `cropPixelGrid`, `recognizeBoard` (crop → recognize → `RecognitionOutcome` with placement, position-search FEN `placement <side> - -` where the side token is `w`/`b` to carry the viewing orientation, `whiteAtBottom`, issues, uncertain squares below confidence 0.45), `flipOutcome` (re-orient without re-recognition; toggles `whiteAtBottom` and the side token). Uncertain squares are those below `LOW_CONFIDENCE` (0.30). |
...
- **Import-from-image reworked into a pinch/pan framing viewport** - The draggable crop frame over a full-height screenshot was replaced by a fixed square viewport the image moves behind: `ImportImageCropSection` is now a single `Canvas` (image drawn via `drawImage(dstOffset, dstSize)` + a fixed 8×8 grid and teal border on top) driven by `detectTransformGestures` — two fingers zoom, one finger pans. `cropRect` stays the single source of truth in image pixels (it *is* the visible region), so the container and `BoardImageImportService.recognizeBoard` call are unchanged; `applyViewportGesture` maps one gesture step onto it, keeping the point under the centroid fixed and clamping the square to `[min(w,h)/MaxZoomFactor, min(w,h)]` and inside the image, so the crop always holds real pixels. `CropHandle`, `resolveCropHandle`, `applyCropDrag` and the dim overlay are gone. Layout: the white-at-bottom setting moved directly under "Choose Image" as a compact `AppToggleRow` (new in `Inputs.kt`, icon-less label+switch variant sharing a private `AppSwitch` with `AppSettingsToggleRow`), "Recognize Position" is pinned in the scaffold `bottomBar` (`RecognizeActionBar`, disabled until an image is picked), the crop hint sits under the viewport, and the result section auto-scrolls into view (`ResultItemIndex = 3` — every section is its own list item and stays in the list while empty so the index holds). `import_position_crop_title` was dropped and `import_position_crop_hint` rewritten for zoom/pan in en/ru/sr.

- **Row-profile terms added to the type metric — the app's own board set now recognises** - `test positions/1000018334.jpg` (full 1080×2400 phone screenshot of this app's editor; board at `x=0..1079, y=260..1339`) was misrecognised on 8 of 64 squares, all systematic by type: every bishop read as a pawn, both queens as rooks, both kings wrong. Cause: `assets/reference_board.png` is a slim Staunton set while the app's canvas renderer draws a fat, flat set whose *filled* silhouettes collapse — bishop and pawn are both "ball on a wide skirt", queen and rook both "wide flat top". 1−IoU is an area ratio, so it is dominated by the body and base, which are near-identical within a set, while the type is written in the head — a few narrow rows carrying little area. Fix: `rowProfile(mask)` (per-row glyph-width share of the normalised mask) plus `profileDistance` / `headProfileDistance`, added to the type distance with `PROFILE_WEIGHT = 1.0` and `HEAD_WEIGHT = 1.0` (`HEAD_ROWS = MASK_SIZE * 2 / 5`). Errors 8 → 2 (d8 queen→rook, g1 king→knight, both flagged uncertain); `suc.png`, `8_2.png` and the reference board stay exact. Weight sweep: profile alone bottomed out at 3 errors, head-only weights ≥ 2.0 broke `suc.png`. **Both confidence thresholds were re-measured because the added terms rescale `1 − distance`**: `MIN_PIECE_CONFIDENCE` 0.35 → 0.10 (real pieces bottom at 0.19, crop debris still 0.00) and `BoardImageImportService.LOW_CONFIDENCE` 0.45 → 0.30 (at 0.45 a cleanly recognised board flagged a third of its own pieces). Guarded by `recognize_appsOwnBoardScreenshot_matchesPositionExceptKnownMisses` (asserts the exact placement including the 2 known misses, so a future fix fails loudly) and `lowConfidenceStaysQuiet_onCleanlyRecognisedBoards`.

- **Import keeps the screenshot's orientation** - A board imported from a black-at-bottom screenshot ("white at bottom" toggle off) is no longer flipped to white-at-bottom in the preview and the position editor. Position search derives board orientation solely from the FEN side-to-move token, so `BoardImageImportService.buildOutcome` now emits `placement b - -` when `whiteAtBottom` is false (`w` otherwise); the recognised placement stays canonical, only the display orientation changes. `RecognitionOutcome` gained a `whiteAtBottom` field; `flipOutcome` toggles it and the side token together. `ImportPositionFromImageScreen` calls `previewController.setOrientation(orientationFor(whiteAtBottom))` alongside every `loadPreviewFen`. Guarded by `BoardImageRecognizerCalibrationTest.recognizeBoard_encodesViewingOrientationInSideToken`.

- **Import-from-image bug fixes (device testing)** - Two defects found running the import flow on the emulator. (1) `BoardImageImportService.decodeBoardImage` always returned null: the `inJustDecodeBounds` pass returns a null Bitmap by contract, and a stray `?: return null` after it treated that as failure, so every picked image failed with "could not load the image". Fixed by dropping the elvis — the existing `bounds.outWidth <= 0` check already covers a failed stream open. (2) On a slightly imperfect manual crop, empty coordinate-labelled squares (e.g. c1/e1 on `suc.png`) were misread as low-confidence knights: a label fragment leaks out of the edge ring into the central region and clears the emptiness threshold. Added `MIN_PIECE_CONFIDENCE = 0.35` in `BoardImageRecognizer.classifyCell` — a match below it is re-classified empty. Sits in the clean gap between real pieces (≥ 0.69 measured) and debris (~0.00), and stays under `LOW_CONFIDENCE`/0.45 so genuine borderline pieces are still surfaced as uncertain. Guarded by `BoardImageRecognizerCalibrationTest.recognize_sucBoard_toleratesImperfectManualCrop`; the `File error accessing recents directory` logcat line is unrelated DocumentsUI noise.

- **Import-position-from-image screen added (plan step 7)** - `ScreenType.ImportPositionFromImage` routed in `MainActivity`; entry is a new `ImportImageIconButton` (in `AppIcons.kt`) in the `PositionSearchScreen` top bar. `ui/screen/positions/importFromImage/ImportPositionFromImageScreen.kt` (Container + Stateless, `ImportPositionFromImageLocalization.kt`, strings in en/ru/sr): pick image via `OpenDocument("image/*")`, drag a crop frame over the screenshot (corners resize, middle moves, 8×8 grid + dim overlay for board alignment), toggle "white at bottom", recognize (`BoardImageTemplates.get` on IO, `BoardImageImportService.recognizeBoard` on Default), preview on a moves-disabled `ChessBoardSection` with warnings (`findPositionIssues` + uncertain squares), then "Open in Position Search" passes the FEN through `runtimeContext.positionSearch.initialFen`. `service/BoardImageImportService.kt` holds decode/crop/recognize/flip logic. Known gaps: uncertain squares are a text list (not board highlights), recognizer issue strings are English-only.

- **`BoardImageTemplates.kt` added — reference board bundled as an asset (plan step 6)** - `test positions/start position cropped.png` copied to `app/src/main/assets/reference_board.png`. `service/BoardImageTemplates.get(context)` decodes it and builds the recognizer's piece templates lazily on first call, caching them for the process lifetime (double-checked `@Volatile`) and recycling the bitmap. `BoardImageRecognizerCalibrationTest.bundledAsset_buildsFullTemplateSet_andRecognisesItself` guards the asset on the plain JVM: it must yield 6 types × 2 colour exemplars and recognise itself as the start position. Remaining plan step 7: import-from-image UI (`ScreenType.ImportFromImage` → crop → recognize → `PositionSearchScreen`).

- **`BoardImageRecognizer.kt` calibrated against real screenshots** - `BoardImageRecognizerCalibrationTest` (plain JVM, ImageIO via reflection) now recognises all three `test positions/` boards exactly: templates from `start position cropped.png`, full piece-placement match on `suc.png` (highlights, in-cell coordinates) and `8_2.png` (flipped, cursor, 800×796). Three recognizer changes came out of calibration: (1) `clearEdgeRing` clears the outer `MASK_SIZE/16` ring of foreground/ink masks and dark ink additionally requires deviation from the per-cell background — border vignettes and neighbour-cell bleed no longer fuse with the glyph; (2) the type distance gained a horizontal-asymmetry term (`maskAsymmetry`, weight 0.5; the knight is the only asymmetric piece — separates fat knights from kings); (3) `pieceIsWhite` decides by the bright-ink share (threshold 0.40) among ink pixels of the glyph core (filled glyph eroded by `MASK_SIZE/12`) — black pieces with white crosses/coronets stay black, hatched white pieces stay white. Stale `rankDigitLooksLikeOne_*` tests are gone. Next steps: reference board into `assets/` + import-from-image UI (plan steps 6-7).
